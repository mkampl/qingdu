...
- **Source**: SUBTLEX-CH-WF, Cai & Brysbaert (2010), *PLoS ONE* 5(6):e10729, DOI 10.1371/journal.pone.0010729. Distributed as free academic download via Ghent University (originally crr.ugent.be, now ugent.be/pp/experimentele-psychologie/en/research/documents/subtlexch).
- **Original licence**: no CC-BY-NC-ND badge exists on the authors' pages, the paper, or the distributed files — that label was a documentation error, corrected 2026-07-02. The base word/character-frequency files are PLOS ONE Supporting Information, which PLOS's own policy publishes under **CC BY** only (no NC/ND riders permitted on anything PLOS publishes). The specific pinyin-annotated combined file QingDu derives from was added to the same academic-download page after the PLOS publication and carries no separate licence statement — it's "free download, please cite us," not a restrictive licence. Precedent: `rspeer/wordfreq` (Apache-2.0) obtained explicit written permission from Brysbaert to redistribute derived SUBTLEX wordlists, including Chinese, for any purpose including commercial use (see its `NOTICE.md`) — the rights holder grants broad permission on request.
- **Bundled file**: `app/data/subtlex_char_pinyin_freq.json` — an aggregated per-(character, pinyin) sum derived from SUBTLEX-CH-WF. No transcript material is bundled. Under CC's own Content Mining FAQ, statistical analysis derived from a dataset is not itself an "adaptation" of that dataset, and raw frequency facts are not independently copyrightable (Feist v. Rural Telephone).
- **Before any commercial distribution** (the planned `qingdu-premium` fork, see [[project-qingdu-llm-features]]): send Brysbaert a short permission email mirroring the wordfreq precedent to close the loop formally. Removing the file is also a clean exit if preferred; pinyin disambiguation falls back to a lower-quality heuristic (already implemented as the absent-data path in `_per_reading_frequency`). A purpose-built, unambiguously-licensed alternative (Apache-2.0) is the CPP dataset from `kakaobrain/g2pM` — narrower coverage but zero licensing questions, worth a future spike if the SUBTLEX dependency is ever worth dropping entirely.
